...
import club.axus.axuspixeldungeon.scenes.IntroScene;
...
import club.axus.axuspixeldungeon.ui.IconButton;
...
public class WndStartGame extends Window {
	...
	public WndStartGame(final int slot){
		...
		if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY)){
			IconButton challengeButton = new IconButton(
				...
				@Override
				protected void onClick() {
					axusPixelDungeon.scene().addToFront(new WndChallenges(SPDSettings.challenges(), true) {
	...
	private static class HeroBtn extends Button {
		...
		protected void onClick() {
			...
			if( !cl.isUnlocked() ){
				axusPixelDungeon.scene().addToFront( new WndMessage(cl.unlockMsg()));
			} else {
	...
	private class HeroPane extends Component {
		...
		protected void createChildren() {
			...
			heroItem = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					axusPixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_item")));
				}
			};
			...
			heroLoadout = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					axusPixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_loadout")));
				}
			};
			...
			heroMisc = new IconButton(){
				...
				protected void onClick() {
					if (cl == null) return;
					axusPixelDungeon.scene().addToFront(new WndMessage(Messages.get(cl, cl.name() + "_desc_misc")));
				}
			};
			...
			heroSubclass = new IconButton(new ItemSprite(ItemSpriteSheet.MASTERY, null)){
				...
				protected void onClick() {
					...
					axusPixelDungeon.scene().addToFront(new WndMessage(msg));
				}
			};
