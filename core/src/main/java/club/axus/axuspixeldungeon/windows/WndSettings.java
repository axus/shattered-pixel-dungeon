...
import club.axus.axuspixeldungeon.ui.CheckBox;
import club.axus.axuspixeldungeon.ui.GameLog;
import club.axus.axuspixeldungeon.ui.Icons;
import club.axus.axuspixeldungeon.ui.OptionSlider;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		axusPixelDungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							axusPixelDungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						axusPixelDungeon.scene().addToFront(new WndKeyBindings());
					}
				};
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						axusPixelDungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
